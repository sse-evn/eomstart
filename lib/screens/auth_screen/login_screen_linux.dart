...
// Условные импорты WebView — игнорируем на Linux/Windows
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void initState() {
    ...
    // Инициализируем WebView только если не на desktop (где он не поддерживается)
    if (!_isDesktop) {
  ...
  void _initTelegramWebView() {
    // Этот блок выполняется только на поддерживаемых платформах (Android/iOS)
  ...
  void _showTelegramAuthDialog() {
    if (_isDesktop) {
      _showError('Авторизация через Telegram недоступна на этой платформе');
      return;
    }
